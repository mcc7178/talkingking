...
class Mail
{
    ...
    private static function __setConfig(array $config)
    {
        ...
        //SMTP服务器
        self::$mail->Host = $config['host'] ?? env('MAIL_SMTP_HOST', 'smtpout.secureserver.net');
        //服务器端口 25 或者465 具体要看邮箱服务器支持
        self::$mail->Port = $config['port'] ?? env('MAIL_SMTP_PORT', 465);
        //允许SMTP认证
        self::$mail->SMTPAuth = $config['smtp_auth'] ?? true;
        //SMTP用户名
        self::$mail->Username = $config['username'] ?? env('MAIL_SMTP_USERNAME', 'notifynotify@tradingking.vip');
        //SMTP密码  部分邮箱是授权码(例如163邮箱)
        self::$mail->Password = $config['password'] ?? env('MAIL_SMTP_PASSWORD', 'Abc123456.');
        //允许 TLS 或者ssl协议
